...
function encontrarLobo(fila) {
    if (fila[fila.length - 1] === "lobo") return "Vete y deja de comer mis ovejas!" 
    let result
    fila.forEach((animal, index) => {
      if (animal === "lobo") result = `Oveja número ${fila.length - (index + 1)}! ¡Estás a punto de ser comida por un lobo!`
    })
    return result
}
